...
async function handlePending(text){const p=STATE.pending;if(!p)return null;p.attempts=(p.attempts||0)+1;const n=norm(text);
  ...
  if(p.stage==='domain'){const d=domainFromReply(n);if(d){const combined=`${p.original} ${text}`;clearPending();record('clarification',text,{resolvedDomain:d});return{reprocess:combined}}if(p.attempts>=3)return'<b>ما زال المجال غير واضح عندي.</b><div class="mut" style="margin-top:6px">اكتب اسم المجال فقط، مثل «المبيعات» أو «خطة التواجد»، وأنا أكمل نفس الطلب بدون ما تعيده.</div>';return optionsHtml('domain')}
