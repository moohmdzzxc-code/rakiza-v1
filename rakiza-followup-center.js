...
.fc-panel{background:#fff;border:1px solid var(--fc-line);border-radius:16px;box-shadow:0 6px 18px #17365d08;padding:12px;margin-top:10px}.fc-create{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;padding:2px 0 12px;border-bottom:1px solid var(--fc-line);margin-bottom:12px}.fc-create-copy b{display:block;color:var(--fc-n);font-size:15px}.fc-create-copy span{display:block;color:#7b8797;font-size:12px;margin-top:3px}.fc-create-actions{display:flex;gap:7px;flex-wrap:wrap}.fc-create-btn{border:1px solid #d8e1ea;background:#fff;color:var(--fc-n);border-radius:10px;padding:9px 12px;font-weight:850;cursor:pointer;display:inline-flex;align-items:center;gap:7px}.fc-create-btn svg{width:17px;height:17px}.fc-create-btn:hover{background:#f5f8fb}.fc-create-btn.short{background:#fff8e8;border-color:#f1ddb0;color:#9d6700}.fc-create-btn.maint{background:#eef6ff;border-color:#d4e5f8;color:#1760b8}.fc-create-btn.action{background:#eefaf3;border-color:#d4eddd;color:#168047}.fc-tools{display:grid;grid-template-columns:minmax(240px,1fr) 2fr;gap:10px;align-items:center}.fc-search{position:relative}.fc-search input{width:100%;box-sizing:border-box;border:1px solid #ccd8e4;border-radius:10px;padding:11px 42px 11px 12px;font:inherit}.fc-search svg{position:absolute;right:13px;top:11px;width:21px;height:21px;color:#17365d}.fc-filters{display:grid;grid-template-columns:repeat(5,1fr);gap:7px}.fc-filter{border:1px solid #dce4ec;background:#f4f7fb;color:#17365d;border-radius:9px;padding:10px 8px;font-weight:800;cursor:pointer;white-space:nowrap}.fc-filter.on{background:#17365d;color:#fff;border-color:#17365d}.fc-filter.late.on{background:#b52626;border-color:#b52626}
...
@media(max-width:1050px){.fc-summary{grid-template-columns:repeat(3,1fr)}.fc-tools{grid-template-columns:1fr}.fc-bottom{grid-template-columns:1fr}.fc-create{align-items:flex-start}.fc-create-actions{width:100%}.fc-create-btn{flex:1;justify-content:center}}
...
function createBar(){return '<div class="fc-create"><div class="fc-create-copy"><b>تسجيل متابعة جديدة</b><span>اختر النوع وسيتم فتح شاشة التسجيل الأصلية بكامل حقولها.</span></div><div class="fc-create-actions"><button class="fc-create-btn short" onclick="fcCreate(\'shortages\')">'+svg('box')+'<span>تسجيل نقص جديد</span></button><button class="fc-create-btn maint" onclick="fcCreate(\'maintenance\')">'+svg('wrench')+'<span>تسجيل طلب صيانة</span></button><button class="fc-create-btn action" onclick="fcCreate(\'actions\')">'+svg('clip')+'<span>تسجيل إجراء جديد</span></button></div></div>'}
...
function render(){
 ...
 b.innerHTML='<div class="fc-hero"><div><h1>مركز المتابعة الموحد</h1><p>كل ما يحتاج متابعة في المعرض من مكان واحد</p></div><div class="fc-target">◎</div></div>'+summary(d)+'<div class="fc-panel">'+createBar()+filters(d)+table()+'</div><div class="fc-bottom">'+attention(d)+distribution(d)+'</div>'
}
window.fcCreate=route=>{
...
ensureView();window.RakizaFollowupCenter={VERSION:'20260925-followup-create-v1'};
})();