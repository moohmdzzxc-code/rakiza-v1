...
function ensureStyle(){
  ...
  const s=document.createElement('style');s.id='rkzExecutiveStyle';s.textContent=`
    ...
    #rkzSidebar{position:fixed;right:0;top:0;bottom:0;width:226px;background:#fff;border-left:1px solid #e8edf4;z-index:25;padding:22px 15px;display:flex;flex-direction:column;box-shadow:-5px 0 24px rgba(23,54,93,.04)}
    .rkz-side-brand{padding:4px 10px 20px;border-bottom:1px solid #eef1f5;margin-bottom:12px}.rkz-side-brand b{display:block;font-size:34px;line-height:1;color:#0f2d52}.rkz-side-brand span{display:block;color:#6f7f94;font-size:12px;margin-top:8px}
    .rkz-side-nav{display:flex;flex-direction:column;gap:5px}.rkz-side-btn{border:0;background:transparent;border-radius:12px;padding:11px 12px;display:flex;align-items:center;gap:11px;color:#213b60;font-weight:750;cursor:pointer;text-align:right;width:100%;transition:.16s}.rkz-side-btn svg{width:20px;height:20px;flex:none}.rkz-side-btn:hover{background:#f3f6fa}.rkz-side-btn.on{background:linear-gradient(135deg,#17365d,#28527d);color:#fff;box-shadow:0 7px 18px rgba(23,54,93,.16)}
    .rkz-side-foot{margin-top:auto;padding:12px 10px;color:#9aa5b4;font-size:11px;text-align:center}
    ...
    @media(max-width:1080px){.rkz-main-grid{grid-template-columns:310px minmax(0,1fr)}.rkz-tools-grid{grid-template-columns:repeat(2,1fr)}.rkz-kpi strong{font-size:24px}}
    @media(max-width:900px){body.rkz-shell .app{margin-right:0;padding:10px}#rkzSidebar{position:static;width:auto;height:auto;border-left:0;border-bottom:1px solid #e5ebf2;padding:10px 12px;box-shadow:none}.rkz-side-brand{display:flex;align-items:center;justify-content:space-between;padding:3px 6px 10px;margin-bottom:8px}.rkz-side-brand b{font-size:26px}.rkz-side-brand span{margin:0}.rkz-side-nav{flex-direction:row;overflow:auto;padding-bottom:2px}.rkz-side-btn{width:auto;min-width:max-content;padding:9px 11px}.rkz-side-foot{display:none}.rkz-kpis{grid-template-columns:repeat(2,1fr)}.rkz-main-grid{display:block}.rkz-panel{margin-bottom:12px}.rkz-alert-panel{min-height:0}.rkz-ai-bar{grid-template-columns:1fr}.rkz-ai-brand{justify-content:center}body.rkz-shell #rakizaCopyright{margin:8px 0 0!important}}
    @media(max-width:600px){.rkz-home-top{align-items:flex-start;flex-wrap:wrap}.rkz-user{order:3;width:100%}.rkz-kpis{grid-template-columns:1fr 1fr}.rkz-kpi{min-height:112px;padding:13px}.rkz-kpi strong{font-size:22px}.rkz-cycle-stats{grid-template-columns:1fr}.rkz-tools-grid{grid-template-columns:1fr}.rkz-ai-ask{flex-direction:column}.rkz-ai-input,.rkz-ai-btn{width:100%}}
    ...
    <div class="rkz-main-grid">
      ...
      <div class="rkz-panel rkz-tools-panel"><div class="rkz-panel-title"><div><h2>الأدوات</h2><p>كل ما تحتاجه لإدارة المعرض في مكان واحد</p></div><div class="rkz-title-icon">${rkzSvg('box')}</div></div>
        <div class="rkz-tools-grid">
          <button class="rkz-tool" onclick="rkzNav('roster')"><span class="rkz-tool-left"><span class="rkz-tool-ico">${rkzSvg('users')}</span>خطة التواجد</span>${rkzSvg('arrow')}</button>
          <button class="rkz-tool" onclick="rkzNav('sales')"><span class="rkz-tool-left"><span class="rkz-tool-ico">${rkzSvg('chart')}</span>المبيعات</span>${rkzSvg('arrow')}</button>
          <button class="rkz-tool" onclick="rkzNav('shortages')"><span class="rkz-tool-left"><span class="rkz-tool-ico gold">${rkzSvg('box')}</span>النواقص</span>${rkzSvg('arrow')}</button>
          <button class="rkz-tool" onclick="rkzNav('maintenance')"><span class="rkz-tool-left"><span class="rkz-tool-ico gold">${rkzSvg('wrench')}</span>الصيانة</span>${rkzSvg('arrow')}</button>
          <button class="rkz-tool" onclick="rkzNav('reports')"><span class="rkz-tool-left"><span class="rkz-tool-ico">${rkzSvg('calendar')}</span>سجل الأيام</span>${rkzSvg('arrow')}</button>
          <button class="rkz-tool" onclick="rkzNav('actions')"><span class="rkz-tool-left"><span class="rkz-tool-ico gold">${rkzSvg('clip')}</span>الإجراءات</span>${rkzSvg('arrow')}</button>
          <button class="rkz-tool" onclick="rkzNav('targets')"><span class="rkz-tool-left"><span class="rkz-tool-ico">${rkzSvg('target')}</span>المستهدفات</span>${rkzSvg('arrow')}</button>
          <button class="rkz-tool disabled" disabled><span class="rkz-tool-left"><span class="rkz-tool-ico">${rkzSvg('chart')}</span>الأداء الأسبوعي</span></button>
          <button class="rkz-tool disabled" disabled><span class="rkz-tool-left"><span class="rkz-tool-ico">${rkzSvg('chart')}</span>الأداء الشهري</span></button>
        </div>
      </div>
    </div>
