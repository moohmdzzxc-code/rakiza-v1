...
ok(loaded[0]==='rakiza-day-cycle.js?v=20260918-1','unified day cycle loads before the dashboard',loaded);
ok(loaded[1]==='rakiza-home-dashboard.js?v=20260918-1','executive dashboard loads after the cycle engine',loaded);
...
ok(loaded[5]==='rakiza-ai-roster-operational.js','operational roster bridge loads before conversation intelligence',loaded);
ok(loaded[6]==='rakiza-ai-conversation.js','conversation intelligence loads before universal router',loaded);
ok(loaded[7]==='rakiza-ai-conversation-universal.js','universal conversation orchestration loads before compatibility',loaded);
ok(loaded[8]==='rakiza-ai-conversation-compat.js','natural roster compatibility remains available',loaded);
ok(loaded[9]==='rakiza-ai-intent.js','Intent Intelligence loads before dialogue resolver',loaded);
ok(loaded[10]==='rakiza-ai-dialogue.js','dialogue capability resolver loads before voice',loaded);
ok(loaded[11]==='rakiza-ai-voice.js','conversational voice loads before execution layers',loaded);
ok(loaded[12]==='rakiza-ai-capabilities.js','unified capability registry loads after voice',loaded);
ok(loaded[13]==='rakiza-ai-orchestrator.js?v=20260918-2','versioned central orchestrator loads last',loaded);
ok(!!sandbox.window.RakizaDayCycle,'unified day cycle is available to the dashboard');
