...
ok(loaded.length===6,'six runtime layers loaded',loaded);
...
ok(loaded[5]==='rakiza-ai-conversation-compat.js','natural roster compatibility loads last',loaded);
...
ok(sandbox.window.askRakizaAssistant.__rakizaUniversalConversationWrapped===true,'universal conversation is outer ask layer');
ok(sandbox.window.askRakizaAssistant.__base?.__rakizaConversationWrapped===true,'approved conversation v1 remains directly below universal layer');
ok(sandbox.window.askRakizaAssistant.__base?.__base?.__rakizaStoreReasoningWrapped===true,'store reasoning remains below conversation');
const frame=sandbox.window.RakizaAI.conversation.interpret('انشاء خطة تواجد محمد صباح والجمعه اجازه اما المذكورة اسماهم فهم مساء واجازتهم بعدهم عمار ثلوث معتوق احد عبدالصبور ربوع مشاري اثنين',{entities:{domain:'attendance'}});
ok(frame.action==='create_roster_draft','screenshot natural roster phrase routes to draft creation',frame);
