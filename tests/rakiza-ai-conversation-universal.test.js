...
function ar(v){return String(v||'').replace(/[أإآ]/g,'ا').replace(/ى/g,'ي').replace(/ة/g,'ه')}
function hasName(v,name){return ar(v).includes(ar(name))}
...
ok(calls.attendance.length===2,'multi-person attendance split into two focused calls',calls.attendance);ok(calls.attendance.some(x=>hasName(x,'عمار مثنى')),'Ammar full name focused',calls.attendance);ok(calls.attendance.some(x=>hasName(x,'معتوق الحارثي')),'Matuq full name focused',calls.attendance);
ok(calls.attendance.every(x=>!(hasName(x,'عمار مثنى')&&hasName(x,'معتوق الحارثي'))),'each specialist call contains one employee only',calls.attendance);
...
await ask('محمد عبده');
ok(!U.state.pendingClarification,'clarification cleared after exact employee');ok(calls.attendance.length===1,'original attendance request resumed after clarification',calls.attendance);ok(hasName(calls.attendance[0],'محمد عبده'),'resumed query focuses clarified employee',calls.attendance[0]);
