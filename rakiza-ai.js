...
const RAI_OPERATIONAL_CONCEPTS={
  ...
  roster:{
    roster:['تواجد','خطة التواجد','جدول الدوام','جدول الأسبوع','جدول الاسبوع','جدول الموظفين','دوام','شفت','شفتات','ورديات'],
    ...
    change:['تغيير التواجد','غير الخطة','عدل الدوام','تعديل الخطة','عدل خطة التواجد','غير التواجد','خلي','بدل']
  },
  tasks:{
...
const RAI_INTENT_LABELS={
  navigate_roster:'فتح خطة التواجد',roster_draft:'إنشاء مسودة خطة تواجد',roster_change_draft:'تعديل خطة التواجد كمسودة تغيير',date_query:'فهم تاريخ/يوم',
...
function raiMonthMentions(text){
...
function raiUsesPriorContext(text){
  const n=raiNorm(text);
...
function raiDetectIntent(text,entities){
  ...
  const navigate=entities.operation==='navigate',create=entities.operation==='create',modify=entities.operation==='modify',imperativeArrange=/^(رتب|سوي|جهز|ابني|اعمل)(?:\s|$)/.test(n);
  if(hasRoster&&navigate)return'navigate_roster';
  if(hasRoster&&modify)return'roster_change_draft';
  if(hasRoster&&(create||entities.constraints.length||raiHas(text,['مسودة'])||imperativeArrange||(!questionLike&&/(?:خطه|خطة|تواجد|دوام|شفت|جدول)/.test(n))))return'roster_draft';
...
function raiRosterChangeReply(a){
...
function raiPeriodLabel(p){
  ...
  if(p.type==='compare_months')return p.months.join(' مقابل ');
...
function raiRespond(a){
  ...
  if(a.intent==='roster_change_draft')return raiRosterChangeReply(a);
